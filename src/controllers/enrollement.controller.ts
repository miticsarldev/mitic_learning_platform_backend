...
export const createEnrollement = async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(400)
      .json({ message: "Erreur lors de la création de l'inscription", error });
  }
};
...
export const getEnrollementById = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const updateEnrollement = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
...
export const deleteEnrollement = async (
  req: Request,
  res: Response
): Promise<void> => {
  try {
    const deletedEnrollement = await Enrollement.findByIdAndDelete(
      req.params.id
    );
...
export const getStudentsByTeacher = async (req: Request, res: Response) => {
  try {
    ...
    if (!teacher || teacher.role !== "teacher") {
      return res
        .status(404)
        .json({ message: "Enseignant non trouvé ou rôle invalide" });
    }
    ...
    if (courseIds.length === 0) {
      return res
        .status(200)
        .json({ message: "Aucun cours créé par cet enseignant", students: [] });
    }
...
export const getEnrollmentProgress = async (req: Request, res: Response) => {
  try {
    ...
    const seriesData = courseTitles.map((courseTitle) => {
      const courseData = enrollmentStats.filter(
        (stat) => stat._id.courseId.toString() === courseTitle
      ); // Comparaison avec les ids des cours
      return {
        name: courseTitle,
    ...
    console.error(
      "Erreur lors de la récupération des données d'inscriptions :",
      error
    );
...
export const getEnrollementsCountByCourseId = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const enrollementsCount = await Enrollement.countDocuments({
      course_id: courseId,
    });
...
export const getEnrollementsByStudent = async (req: Request, res: Response) => {
  try {
    ...
    if (!user_id) {
      return res
        .status(400)
        .json({ message: "L'ID de l'étudiant est requis." });
    }
    ...
    if (!enrollements.length) {
      return res
        .status(404)
        .json({ message: "Aucun enrôlement trouvé pour cet étudiant." });
    }
